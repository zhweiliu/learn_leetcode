from typing import List, Dict, Set

'''
https://leetcode.com/explore/interview/card/top-interview-questions-medium/107/linked-list/783/

You are given two non-empty linked lists representing two non-negative integers. The digits are stored in reverse order, 
and each of their nodes contains a single digit. Add the two numbers and return the sum as a linked list.

You may assume the two numbers do not contain any leading zero, except the number 0 itself.

Constraints:
    - The number of nodes in each linked list is in the range [1, 100].
    - 0 <= Node.val <= 9
    - It is guaranteed that the list represents a number that does not have leading zeros.

    

Example 1:
Input: l1 = [2,4,3], l2 = [5,6,4]
Output: [7,0,8]
Explanation: 342 + 465 = 807.

Example 2:
Input: l1 = [0], l2 = [0]
Output: [0]

Example 3:
Input: l1 = [9,9,9,9,9,9,9], l2 = [9,9,9,9]
Output: [8,9,9,9,0,0,0,1]

'''


# Definition for singly-linked list.
class ListNode:
    def __init__(self, val=0, next=None):
        self.val = val
        self.next = next


class Solution:

    def addTwoNumbers(self, l1: ListNode, l2: ListNode) -> ListNode:
        if not l1:
            return l2

        if not l2:
            return l1

        into_next_weight = False

        root = ListNode(val=0)
        node = root

        while l1 or l2:
            l1_num = l1.val if l1 else 0
            l2_num = l2.val if l2 else 0

            num = l1_num + l2_num
            if into_next_weight:
                num += 1
                into_next_weight = False

            into_next_weight = int(num / 10) == 1
            num %= 10
            node.next = ListNode(val=num)
            node = node.next

            l1 = l1.next if l1 else None
            l2 = l2.next if l2 else None

        if into_next_weight:
            node.next = ListNode(val=1)

        return root.next


def create_link_list(nums: List[int]) -> ListNode:
    if len(nums) == 0:
        return None

    node = ListNode(val=nums[0])
    root = node

    for i in range(1, len(nums), 1):
        node.next = ListNode(val=nums[i])
        node = node.next

    return root


if __name__ == '__main__':
    n1 = [9,9,9,9,9,9,9]
    n2 = [9,9,9,9]

    l1 = create_link_list(n1)
    l2 = create_link_list(n2)

    sol = Solution()
    sum_node = sol.addTwoNumbers(l1, l2)

    nums = []

    while sum_node:
        nums.append(sum_node.val)
        sum_node = sum_node.next

    print(nums)
